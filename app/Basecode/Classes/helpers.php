...
function sendOtp($mobile_no, $message = '') {
    ...
    if(!$message) $message = 'Please enter '.$otp.' as your one time verification for mobile number. Team Diagno Labs';
...
if (! function_exists('sendPushNotification')) {
    ...
    function sendPushNotification( $userIds, $data, $debug = false ) {
        ...
        $key = Appconfig::get_config_value('pushnotification', 'firebase_api_user1');

        $deviceTokens = \App\DeviceTokens::whereIn('user_id', $userIds)->where('device_type', 'android')->pluck('device_token');

        sendAndroidPushNotification($deviceTokens, $data, $key);

        // ios notification
        $deviceTokens = \App\DeviceTokens::whereIn('user_id', $userIds)->where('device_type', 'ios')->pluck('device_token');
...
function getColorsArr() {
    return [
        ...
        'black' => ucfirst('black'),
        'green' => ucfirst('green'),
        'silver' => ucfirst('silver'),
        'lime' => ucfirst('lime'),
        'gray' => ucfirst('gray'),
        'olive' => ucfirst('olive'),
        'white' => ucfirst('white'),
        'yellow' => ucfirst('yellow'),
        'maroon' => ucfirst('maroon'),
        'navy' => ucfirst('navy'),
        'red' => ucfirst('red'),
        'blue' => ucfirst('blue'),
        'purple' => ucfirst('purple'),
        'teal' => ucfirst('teal'),
        'fuchsia' => ucfirst('fuchsia'),
        'aqua' => ucfirst('aqua'),
    ];
}

function getShortTd( $content ) {

    return '<p data-toggle="tooltip" title="'.$content.'" class="text-primary">'.str_limit($content, 15).'</p>';
...
function getMonthArr() {
    ...
    foreach (range(1,12) as $month) $arr[$month] = date('F', strtotime('01.'.$month.'.2001'));
